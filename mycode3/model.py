...
class Model(object):
    ...
    def train(self) -> None:
        ...
        for step in range(self.training_steps):
            ...
            loss_dict['AE'] = self.compute_vae_loss(x_A, x_Arecon) + \
                               self.compute_vae_loss(x_B, x_Brecon)
            loss_dict['LA'] = self.compute_latent_align_loss(z_A, z_AtoB) + \
                              self.compute_latent_align_loss(z_B, z_BtoA)
            
            """
            W_A = KNN_Binary_Matrix_torch(z_A, nearest_neighbor=30)
            W_B = KNN_Binary_Matrix_torch(z_B, nearest_neighbor=30)
            z_Adist = pairwise_euclidean_distance(z_A, z_A, clip=False)
            z_Bdist = pairwise_euclidean_distance(z_B, z_B, clip=False)
            loss_dict['Geo'] = torch.sum(W_A*z_Adist)/torch.sum(W_A) + torch.sum(W_B*z_Bdist)/torch.sum(W_B)
            #loss_dict['Geo'] = generalized_clip_loss_stable(z_A, z_A, W_A) + generalized_clip_loss_stable(z_B, z_B, W_B)
            """
            loss_dict['Align'], loss_dict['CLIP'], loss_dict['Geo'] = self.compute_ot_loss(z_A, z_B, batch_A, batch_B)
    ...
    def compute_vae_loss(self, x: torch.Tensor, x_recon: torch.Tensor) -> torch.Tensor:
    ...
    def compute_latent_align_loss(self, z: torch.Tensor, z_to: torch.Tensor) -> torch.Tensor:
        return F.mse_loss(z, z_to, reduction='mean')
    ...
    def compute_ot_loss(self, z_A: torch.Tensor, z_B: torch.Tensor, batch_A: dict, batch_B: dict) -> torch.Tensor:
        ...
        if self.use_mnn:
            mnn = build_mnn_prior(c_cross, self.n_KNN)
        else:
            mnn = None

        T = unbalanced_ot(cost_pp=c_cross, reg=0.05, reg_m=0.5, prior=mnn, device=self.device)
        ...
        return loss_align, loss_clip, loss_geo
