...
class AnnDataDataset(Dataset):
    def __init__(
        ...
        self.celltypes = self._extract_obs_column(adata, celltype_key) if celltype_key else None
        self.sources = self._encode_source_column(adata, source_key) if source_key else None
        self.link_feat = (
            self._convert_to_tensor(adata.obsm["link_feat"])
            if "link_feat" in adata.obsm
            else None
        )

    def _extract_obs_column(
        self,
        adata: AnnData,
        key: Optional[str],
    ) -> Optional[np.ndarray]:
